### THESE TESTS VERIFY THAT RESULTS FROM 2016 BIOMASS CALCULATIONS MAKE SENSE
### ALL LINES OF CODE AT THE BOTTOM OF THIS SCRIPTS SHOULD RETURN "TRUE" IF THE RESULTS ARE OK
### *NOTE*: Must load drought data frame. Code for loading it is in LEMMA_ADS_AllSpp_2016_Turbo.R 
...
result.lemma.p <- read.csv(file = "LEMMA_ADS_AllSpp_2016_Turbo_01242016.csv", header=T) # This takes a while! 
...
# Test NO_TREES1S
a <- unique(testplot$Pol.NO_TREES1)[1] 
...
as.factor(unique(testplot$Pol.x)) == as.factor(coordinates(single)[1])
as.factor(unique(testplot$Pol.y)) == as.factor(coordinates(single)[2])
as.factor(unique(testplot$Pol.NO_TREES1)) == as.factor(sum(as.numeric(paste((test1$relNO))))) # THIS SHOULD BE TRUE
