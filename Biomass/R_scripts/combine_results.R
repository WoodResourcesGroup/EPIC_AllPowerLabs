...
for(i in 1:length(unit.names)){
  for(j in 1:length(YEARS)){
    ...
    assign(paste(unit.names[i],"_sum_BM_",YEARS[j],sep=""), sum_D_BM_Mg)
  }
}

# Put summed BM into table
for(i in 1:length(unit.names)){
  df[unit.names==unit.names[i],"BM_2016_tot"]<- get(paste(unit.names[i],"_sum_BM_", "2016", sep=""))
}

for(i in 1:length(unit.names)){
  df[unit.names==unit.names[i],"BM_1215_tot"]<- get(paste(unit.names[i],"_sum_BM_", "1215", sep=""))
}

# Calculate Mg/ha and percent change

df$BM_D_Mgha_1215 <- df$BM_1215_tot/df$L.area.ha
df$BM_D_Mgha_2016 <- df$BM_2016_tot/df$L.area.ha
df$BM_D_Mhga <- df$BM_D_Mgha_1215+df$BM_D_Mgha_2016
df$Perc_D_25 <- df$BM_D_Mhga/df$BM25 
