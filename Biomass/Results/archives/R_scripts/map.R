...
for(i in 1:length(units)){
  ...
  print(max((na.omit(raster@data@values)/1000)*(900/10000)))
}
breakpoints <- c(0,2,4,6,8,10,12)
colors <- c("yellow","orange","red","brown","chocolate4","black")
plot(units)
plot(LTMU, add=T)
plot(KCNP, add=T)
plot((ENF_raster_2016/1000)*(900/10000),breaks=breakpoints,col=colors, add=T, legend.shrink=1, legend.width=2, zlim=c(0, 1),
     axis.args=list(at=pretty(0:1), labels=pretty(0:1)),
     legend.args=list(text='Dead biomass, Mg/ha', side=4, font=2, line=2.3))
plot((SNF_raster_2016/1000)*(900/10000),breaks=breakpoints,col=colors, add=T, legend=F)
plot(SQNP_raster_2016,breaks=breakpoints,col=colors, add=T, legend=F)
plot((KCNP_raster_2016/1000)*(900/10000)),breaks=breakpoints,col=colors, add=T, legend=F)
plot(LNP_raster_2016,breaks=breakpoints,col=colors, add=T, legend=F)
plot(LTMU_raster_2016,breaks=breakpoints,col=colors, add=T, legend=F)
plot(ENF_raster_2016,breaks=breakpoints,col=colors)
