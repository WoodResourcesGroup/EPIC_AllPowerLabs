#########################################################################################################################
######## CREATE WHOLE PROCESS BY UNIT THAT CAN BE USED ON ANY UNIT BY DEFINING WHICH ONE YOU WANT EARLY ON
#########################################################################################################################
...
names(results)[names(results)=="V5"] <- "relBA"
names(results)[names(results)=="V6"] <- "PlotID"
...
pts.intersect <- strsplit(dimnames(KCNP.intersect@coords)[[1]], " ")
pts.intersect.id <- as.numeric(sapply(pts.intersect,"[[",2))
pts.extract <- spdf[pts.intersect.id, ]
mask <- subset(KCNP_16, KCNP_16$key %in% KCNP.pts.intersect.id)

### Check that you masked it correctly
plot(mask, add=T, col="pink", pch=".")
plot(drought, add=T)
