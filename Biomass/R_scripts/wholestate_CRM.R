...
for(j in 1){#:2){
  ...
  save(spdf, file=paste("Results_",YEAR,"_WS_25_CRM.Rdata", sep=""))
  save(results, file=paste("Table_",YEAR, "_","WS_25_CRM.Rdata", sep=""))
  ### Save version masked to just the management unit
  ## Convert to raster to more easily crop and sum
  write.csv(results, file=paste("Results_", YEAR,"_WS_25_CRM.csv",sep=""), row.names = F)
  print(Sys.time()-strt)
}
